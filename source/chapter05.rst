...
表达式实际上创建的是个进程（ procedures ），或者更确切的说，一个可被调用的实例（ AMOP ）。
这有什么区别呢？过程不一定与值的映射有任何关系，而是可执行的代码片段（函数调用了的），
可能是保存的环境变量。当 lisp 程序员以某种称作 *函数式风格* 编写代码时，之后生成过程组合一起假装是，并可能被认为是数学风格的函数映射。
...
是，将其视为任意旧的常规的值。一个口号出现了，为这个下一步的巨大抽象的推进动员程序员： a society without classes , first-class procedures （一个没有类的社会，头等的就是进程）。与那些将进程降
级到次等二级类的语言相比，lisp ，这种一直拥有这些以进程为头等的语言，似乎更加是函数式
的。

最后，大部分语言为了支持某些糟糕的 _Blub_ 语法（如中缀赋值），在表达式和语句之间
做了无意义的区分。在 lisp 中，所有内容都会有所返回（除了 (values) 什么都不返回。但即使这也被强制为 nil ，因此可以在表达式中使用。），同时对代码的嵌套和组合也没有任
...
**terpri** 和 **format** 这样的 lisp 进程会产生指向终端或文件的输出（在 **terpri** 情况
...
宏设计的一个重要原则就是自上而下编程。在设计一个 lisp 宏时，你想要首先要从抽象开始。你想要这样编写程序：
...
这种链式的单位就感觉自然了。分钟依据我们的基础单位，秒，小时基于分钟，天基
...
四舍五入的影响，并让 lisp 使用尽可能精确的算法。例如,弗隆（ furlong ）正相当于 1/8 英
...
**defunits%** 用了嵌套了的反引号（ **`** ）：一个非常难以理解的结构。用反引号编程就像在代码
...
然而，这些宏并不完整。 **defunits%** 宏不支持链式。 **defunits%%** 支持链式，但缺少错误检查。专业的宏编写人员总是小心地处理任何可能出现的错误条件。在无限循环或是
...
行表达式的主体并返回最后的那个结果。 **defun** 会在表达式外添加隐式的 **lambda** ，因此不需
要在已命名的函数中使用 lambda 格式。

本节介绍的是本书中后面要用到的遍历代码的宏—— **tree-leaves** （另请参阅 COMMON LISP 函数 subst 。）的派生以及构造。和
...
该函数在 **tree** 表达式中递归，然后构造出一个新的具有相同形状的列表结构。
...
的就不用去管他了。在 lisp 中，编写一个可自定义的使用函数的最直接的方法就是有插
...
有宏组合（ **macro composition** ）那么有趣（这就是为什么函数组合只得到本书的几个段落而宏组合却得到本书大部分内容的原因。），但这仍是个很有用的技术，且这个
技术是专业 lisp 程序员必须掌握的。
...
声明一个变量是可忽略后再使用这个变量并没有什么危害（ Lisp 会发现它实际上是不容忽视的。），因此可以将两个变量 **x**
都声明为可忽略：
...
一个隐式词法变量绑定。这种不可见变量的引入被认为违反了词法透明性。另一种说法是，
...
**defmacro** 定义全局的宏一样。就像 lisp 会在代码中展开成 **defmacro**-定义的宏，当它
代码遍历你的表达式， **macrolet**-定义宏也会被展开。
...
不止23 个这种的特殊结构（ ANSI CL 中有 25 种特殊形式，23 种没有 let 和 quote 。），这些结构也需要添加特定的逻辑。更重要的是，许多特殊结构复杂到
...
形参都使用 **gensym** ，来避免不想要的捕获，以及 **loop** 宏（很奇怪 Loop 是 COMMON LISP 中最具争议的问题之一。然而，大多数反对它是完全没有根据的。 Loop 是一种非常方便的领域特定语言，用于循环领域。）汇集这
些 **gensyms** 。
...
绑定，然后跳转回“命名的 let （ named let ）”的顶部。因此 **nlet-tail** 接受提供了的“命名的 let （ named let ）”，我们上面示例中的 fact ，然后创建一个局部宏，该宏只在提供的代码主体中生效。这个宏展开的代码，使用
**psetq** 将 **let** 的绑定设为提供的新的值，然后跳转回顶部，不需要堆栈空间。最重要
的是，我们可以在我们的程序中为其他不相关的宏使用 **fact** 这个名字（不管什么样类型的编程书怎么会没有几个阶乘实现？）。
...
控制程序计数器，在我们正在执行的代码的当前位置，我们可以创建很有效的宏展开式。虽然在现代高级语
...
**cadr** 和 **second** 效果是一样的：获取列表中的第二个元素。不同之处在于 **second** 是以它做的事命名，而 **cadr** 是以它如何做事来命名的。 **cadr** 是显然的具体说明，而
**second** 是个容易记住的函数名，但它不合需要地掩盖了操作的含义（特别是因为 second 与 cadr 完全相同：您不能使用它来获取其他序列（如向量）的第二个元素。）。 易读的具体说明通常更好，
...
的。 **nthcdr** 也一样，只是它不做最后的 car ： **(nthcdr 2 list)** （由于 COMMON LISP 的第二个命名空间，我们可以轻松地使用 list 作为变量名。这样的例子在像 Scheme 这样的单命名空间 lisp 中会出现问题。）和 **(cddr list)** 是一样的。
...
A 或 D 字符被替换为 X (除了本质上，cxr 与 Maclisp 中访问大块插槽的 cxr 无关。)。 在 **cxr%** 中，第一个参数是个列表，列表中指定这些了 A 和 D。这个
...
本节的重点是说明递归展开的实际用途。当一个宏将一个形式展开位将一个结构展开为一个新的结构时，递归展开就会出现。 与所有递归一样，此过程必须有个基本的终止条件。宏最终会展开
...
下面这个递归的结果展示了 **xcr%** 另一种可能的用法：空列表访问器（如果 COMMON LISP 包含这个，它可能被称为 cr 。）：
...
求解所提供的内容，并将此值（希望这个值应该是一个数字。在 lisp 中，我们可以放心地将这种情况留给 lisp 的异常系统来处理和描述给程序员。）用作要遍历的 **cars** 或 **cdrs** 的数量。

**cxr%** 的第二个问题是，当给到特别巨大的数字作为 A 和 D 符号的前缀时， **cxr%** 会内联（ inline ）所有的 **car**
...
因为编写宏是个迭代的、分层次的过程，我们经常被鼓励组合或结合我们之前实现的宏。
...
lisp ，所以我们可以在宏定义中受益像 **loop** 和 **format** （ format 是 COMMON LISP 的一个颇有争议的特性。然而，就像对 loop 的反对意见一样，大多数都是基于对领域特定语言的概念和范围的误解。）这样的高级特性。
...
LISP 中被定义了（重新绑定 COMMON LISP 指定的函数是被禁止的。）。
...
转换为一个可以用作 **cxr** 第一个参数的列表（有趣的是，已弃用的函数 explode 可能在这种情况下被证明是有用的，但却被 COMMON LISP 排除在外，因为没有人能想到它有什么好的用途。）。下面是它的用法示例：
...
数，然后用 labels 形式将这些函数绑定到提供的代码周围（这种方法的一个问题是此类访问器将不会是可设置的。）。
...
注意，我们已经选择了关键字符号，也就是以冒号 **:** 开头的符号，并且总是求解为自身用来
指示消息。关键字很方便，因为我们不需要引用它们或将它们从包中导出，而且也很直观，因为它们
就是被设计用来执行这个和其他的各种解构。通常在 **lambda** 或 **defmacro** 结构中，关键字
在运行时不会被解构。但是由于我们正在实现一个消息传递系统，其是一类运行时解构，
...
**dlambda** 名称的来源：它是 **lambda** 的解构或分发版本。
...
的列表实际上是 lambda 解构列表。每个分发实例，或者说每个关键字参数，都可以有它自身特定
...
对象的术语编程，就像我们正在创建一个具有多方法的对象一样。 **dlambda** 被定制，为了在不偏离

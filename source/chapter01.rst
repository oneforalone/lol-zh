...
甚至还有思想的表达。他认为把作者与读者之间交流的语言中的概念抽象出来是有必要的，并
把它称为 U 语言。
...
::
  ...
  他大多数人的语言和我们的工作更密切相关的事实，那么引起人们对它的关注是没有意义的。

贯穿全书，我们将使用 *斜体* 来表示一些关键的概念和要点。用 **粗体** 来表示程序中的特殊结构、函数、宏和其他的标识符，不论它们有没有出现过。注意有些词有多种含义，例如 **lambda**
是 COMMON LISP 的宏，而 *lambda* 是概念； **let** 既是特殊操作符，也是作为 **let** 形
式体的列表 。
...
望展示一点代码的使用，或者只是想去讨论一些与书面文本不脱离太多的表达式的特性。在这些情况下，代码或代码的使用示例将像这样出现
...
首先试图不使自己（或任何东西）太严肃。这里有幽默，不同的是，你需要去寻找它。
...
Curry 将宏描述为“元编程（ metaprogramming ）”。元编程的唯一目的是使程序员能够更好地编
写软件。尽管元编程在所有的编程语言都被不同程度地采用，但没有一种语言像 lisp 那
样彻底地采用了它。没有其他语言中的程序员需要用编译元编程技术的方式写代码。这就是
为什么 lisp 程序在非 lisp 程序员看来很奇怪： lisp 代码如何表达是其元编程需求的直接
结果。正如本书试图描述的那样， lisp 的这一设计决定--在 lisp 本身中编写元程序，使得
...
量采用他作品中的其他约定。但 Curry 对逻辑和元编程的贡献至今仍激励着我们。这不仅是因为他对符号引文有深刻的见解，而且还因为他的 U 语言措辞优美，执行高效。
...
的意义上讲， lisp 是由元原语（ meta-primitive ）组成的。一旦宏像在 COMMON LISP 中那样被
标准化，语言的其他部分就可以从根本上被“自举”起来了。大多数语言只是试图提供一套
...
C 或 lisp 编译器来处理。但是 lisp 用户要求对其他几乎所有的一切进行控制。就给予程序员
的控制权而言，没有其他语言能像 lisp 那样彻底。

听从 OnLisp_ 的建议，本书是作为洋葱外的另一层设计的。就像程序在其他程序上分层
一样，本书也是 OnLisp_ 外的又一层。 Graham 的书的中心主题是：当设计良好的"实用工具"结合
...
最佳方式。由于我们经常使用这种功能，所以尽可能通用地创建其抽象是有意义的。 Graham 的
...
表达式中某些对象的存在， **flatten** 完成了一种代码遍历（ code-walking ），这是本书中反复出现的主题。
...
**fact** 和 **choose** 是阶乘和二项式系数函数的很明显的实现。
